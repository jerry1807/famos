					...
					<div class="chat-sidebar-messages">
						@foreach ($messages as $key => $message)
							<div class="chat-sidebar-message @if ($loop->first) selected-message @endif" id="{{ $message->message_code }}">
								<h6 class="chat-title" id="title-{{ $message->message_code }}">
									{{ __($message->title) }}
								</h6>
								<div class="chat-info">
							...
							<div class="col-sm-12">	
								...
								<div class="input-box mb-0">								
									<div class="chat-controllers">						    
										<textarea type="message" class="form-control @error('message') is-danger @enderror" rows="2" id="message" name="message" placeholder="{{ __('Type your message here...') }} ({{ __('Press Shift + Enter to send') }})"></textarea>
										{{-- <div><a class="btn chat-button-icon" href="javascript:void(0)" id="prompt-button"><i class="fa-solid fa-notebook"></i></a></div> --}}
	...
	function process(message) {
		...
		fetch('/user/chat/process', {
			...
			.then(data => {
				...
				eventSource = new EventSource("/user/chat/generate?message_code=" + active_id);				
